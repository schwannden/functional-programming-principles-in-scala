...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences =
    val unsortedMap = w.toLowerCase groupBy identity map {case (k, v) => k -> v.length}
    unsortedMap.toList.sortBy (_._1)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    (y foldLeft Map[Char,Int]() ++ x) {case (acc, (c, freq)) =>
      val nfreq = acc(c) - freq
      if (nfreq > 0)
        acc + (c -> nfreq)
      else
        acc - c
   ...
   */
  def sentenceAnagrams(sentence: Sentence, debug: Boolean = false): List[Sentence] =
    ...
    val result = iter( sentenceOccurrences(sentence) )
    result
