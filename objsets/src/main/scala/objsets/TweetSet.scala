...
class Empty extends TweetSet:
  ...
  override def leastRetweetedAcc(acc: Tweet): Tweet = acc
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    ...
    this.right.filterAcc(p, this.left.filterAcc(p, newAcc))

  override def union(that: TweetSet): TweetSet =
    this.left.union(this.right.union(that.incl(elem)))

  override def mostRetweeted: Tweet =
    this.left.mostRetweetedAcc(this.right.mostRetweetedAcc(elem))

  override def leastRetweeted: Tweet =
    this.left.leastRetweetedAcc(this.right.leastRetweetedAcc(elem))

  override def mostRetweetedAcc(acc: Tweet): Tweet =
    if (elem.retweets > acc.retweets)
      this.left.mostRetweetedAcc(this.right.mostRetweetedAcc(elem))
    else
      this.left.mostRetweetedAcc(this.right.mostRetweetedAcc(acc))

  override def leastRetweetedAcc(acc: Tweet): Tweet =
    if (elem.retweets < acc.retweets)
      this.left.leastRetweetedAcc(this.right.leastRetweetedAcc(elem))
    else
      this.left.leastRetweetedAcc(this.right.leastRetweetedAcc(acc))

  override def descendingByRetweetAcc(acc: TweetList): TweetList =
    val leastRetweeted = this.leastRetweeted
    this.remove(leastRetweeted).descendingByRetweetAcc(Cons(leastRetweeted, acc))
